...
```bash
# Set the stable version (must match the release, e.g. 2.3.2 from 2.3.2-beta.0)
npm run version:stable -- 2.3.2
...
- Users on `ngxsmk-datepicker@latest` (or `ngxsmk-datepicker`) will get the latest stable (e.g. 2.3.2)
- Users on `ngxsmk-datepicker@beta` will continue to get the latest beta until you publish a new one

### Manual version (optional)

- Set a specific beta: `node scripts/set-beta-version.js 2.3.2-beta.0`
- Set stable: `node scripts/set-stable-version.js 2.3.2`
