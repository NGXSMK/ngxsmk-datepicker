...
```bash
# Bump to next beta (e.g. 3.0.0 → 3.0.0-beta.0; run again for 3.0.0-beta.1)
...
```bash
# Set the stable version (must match the release, e.g. 3.0.0 from 3.0.0-beta.0)
npm run version:stable -- 3.0.0
...
- Users on `ngxsmk-datepicker@latest` (or `ngxsmk-datepicker`) will get the latest stable (e.g. 3.0.0)
- Users on `ngxsmk-datepicker@beta` will continue to get the latest beta until you publish a new one

### Manual version (optional)

- Set a specific beta: `node scripts/set-beta-version.js 3.0.0-beta.0`
- Set stable: `node scripts/set-stable-version.js 3.0.0`
